...
# Libraries ---------------------------------------------------------------
pacman::p_load(limma,knitr,venn,RColorBrewer,ggplot2,ggplotify,patchwork,Hmisc,ggpubr,pheatmap,tidyr,tibble,dplyr,ggprism,Seurat, tidyverse)
...
umap_arial_text_min_8 <- theme(
  text = element_text(family = "Arial", size = umap_min_text_size),
  axis.text = element_text(family = "Arial", size = umap_min_text_size),
  axis.title = element_text(family = "Arial", size = umap_min_text_size),
  strip.text = element_text(family = "Arial", size = umap_min_text_size),
  legend.text = element_text(family = "Arial", size = umap_min_text_size),
  legend.title = element_text(family = "Arial", size = umap_min_text_size),
  plot.title = element_text(family = "Arial", size = umap_min_text_size)
)
```

```{r scriptsFunction, include = F}
# source(here::here("scripts", "common", "general_functions.R"))
...
combined@meta.data <- combined@meta.data |>
  mutate(
    source = str_extract(source_name_ch1, "(?<=\\d\\s)\\w.+"),
    source = if_else(source == "Unaffected Lesion", "Unaffected Skin", source)
  )

set.seed(111)

source_cells <- combined@meta.data |>
  tibble::rownames_to_column("cell") |>
  filter(!is.na(source))

source_target_n <- source_cells |>
  count(source) |>
  pull(n) |>
  min()

source_cells_balanced <- source_cells |>
  group_by(source) |>
  slice_sample(n = source_target_n) |>
  ungroup() |>
  pull(cell)

combined_balanced <- subset(combined, cells = source_cells_balanced)

sc_skin_all.cells <- Seurat::DimPlot(
    combined_balanced,
    group.by = "Identity",   # cluster identities
    split.by = "source",  # metadata column with "EM Lesion" vs "Unaffected Skin"
    label = TRUE,
    pt.size = 0.1,
    label.size = umap_geom_text_size,
    combine = FALSE
)

sc_skin_all.cells <- sc_skin_all.cells |>
  purrr::map(
    ~ .x +
 ...
 ggsave(
  here::here("fig_7_files", "figure_07a_skin_scrnaseq_umap_lesion_status.png"),
  sc_skin_all.cells,
  device = ragg::agg_png,
  width = 3.6, height = 2.91,
  units = "in", res = 300
  )
```
...
load(
  file = here::here(
    "data", "intermediate", "03_public_skin_rna", "ncells.RData"
  )
)
...
forest_plot_skin_main <- results_skin_glm |> 
...
ggsave(
  here::here("fig_7_files", "figure_07b_skin_cell_type_abundance_forest.png"),
  forest_plot_skin_main,
  scaling = 2.5,
  width = 9,
  height = 8,
  device = ragg::agg_png)
...
```{r data}
load(here::here("data", "processed", "03_public_skin_rna", "combined3.RData"))
load(here::here("data", "intermediate", "03_public_skin_rna", "SubCluster_Labels.RData"))
...
rm(list = base::intersect(
  c(
    "labs", "source_cells", "source_target_n", "source_cells_balanced",
    "combined_balanced", "sc_skin_all.cells", "n_cells", "cell_type",
    "data_skin_binary", "results_skin_glm", "g1", "g2",
    "forest_plot_skin_main", "labels", "clean_meta"
  ),
  ls()
))
```
...
rm(list = base::intersect(
  c("alternative_labels", "cts", "cts.t", "splitRows", "cts.split"),
  ls()
))
```
...
# Check if paired
  if (.y == TRUE) {
    DESeq2::DESeqDataSetFromMatrix(countData = countData_list[[.x]],
...
pngfile <- here::here("fig_7_files", "figure_07c_skin_scrnaseq_marker_heatmap.png")
...
pngfile <- here::here("fig_7_files", "figure_07_exploratory_kegg_enzyme_volcano.png")
...
de_enzymes_skin3 <- de_enzymes_skin2 |>
  # Preserve the original DESeq-style names used by the heatmap code below.
  mutate(
...
pngfile <- here::here("fig_7_files", "figure_07d_skin_kegg_enzyme_heatmap.png")
...
load(here::here("data", "intermediate", "03_public_skin_rna", "Public Data Limma.RData"))
